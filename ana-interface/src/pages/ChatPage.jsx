...
function ChatPage() {
  ...
  const getFileCategory = (file) => {
    ...
    const imageExts = ['png', 'jpg', 'jpeg', 'webp', 'gif', 'bmp', 'ico', 'svg', 'tiff', 'tif', 'avif'];
    const codeExts = ['js', 'cjs', 'mjs', 'ts', 'tsx', 'jsx', 'py', 'java', 'cpp', 'c', 'h', 'hpp', 'go', 'rs', 'rb', 'php', 'swift', 'kt', 'scala', 'lua', 'r', 'pl', 'sh', 'bat', 'ps1', 'sql', 'vue', 'svelte'];
    const configExts = ['json', 'yaml', 'yml', 'xml', 'ini', 'cfg', 'conf', 'toml', 'env', 'csv', 'tsv', 'log', 'dat'];
    const docExts = ['pdf', 'txt', 'md', 'doc', 'docx', 'rtf', 'odt'];
  ...
  const handleFileUpload = async (files) => {
    ...
    for (const file of fileList) {
      ...
      if (!category) {
        addSystemMessage(`❌ Format non supporté: ${file.name}. Extensions acceptées: images, documents, code, config, web`, 'error');
        continue;
      }
      ...
      try {
        ...
        // Documents (PDF, TXT, MD, DOC, DOCX, RTF, ODT)
          ...
          if (ext === 'pdf') {
            ...
            addSystemMessage(`⚠️ ${ext.toUpperCase()}: Extraction basique. Pour meilleur résultat, convertir en PDF.`, 'warning');
            textContent = `[Document ${ext.toUpperCase()}: ${file.name} - Convertir en PDF pour extraction complète]`;
          }
          else {
            textContent = await readTextFile(file);
            addSystemMessage(`📄 Document uploadé: ${file.name} (${(file.size / 1024).toFixed(1)} KB)`);
          }
        ...
        // Code files
          ...
          addSystemMessage(`💻 Code uploadé: ${file.name} (${(file.size / 1024).toFixed(1)} KB) [${ext.toUpperCase()}]`);
        }
        // Config/Data files
          ...
          addSystemMessage(`⚙️ Config uploadé: ${file.name} (${(file.size / 1024).toFixed(1)} KB) [${ext.toUpperCase()}]`);
        }
        // Web files (HTML, CSS, etc.)
          ...
          addSystemMessage(`🌐 Fichier web uploadé: ${file.name} (${(file.size / 1024).toFixed(1)} KB) [${ext.toUpperCase()}]`);
        }
      } catch (error) {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    accept: {
      'image/*': ['.png', '.jpg', '.jpeg', '.webp', '.gif', '.bmp', '.ico', '.svg', '.tiff', '.tif', '.avif'],
      'application/pdf': ['.pdf'],
      'text/*': ['.txt', '.md', '.html', '.htm', '.css', '.scss', '.sass', '.less', '.js', '.cjs', '.mjs', '.ts', '.tsx', '.jsx', '.json', '.xml', '.yaml', '.yml', '.csv', '.tsv', '.log', '.ini', '.cfg', '.conf', '.toml', '.env', '.sh', '.bat', '.ps1', '.sql', '.py', '.java', '.c', '.cpp', '.h', '.go', '.rs', '.rb', '.php', '.swift', '.kt', '.vue', '.svelte'],
              ...
              <div className="message-text">
                {message.streaming && !message.text ? (
                  <span className="loading-spinner">
                    <span className="spinner-icon"><IconLoader2 size={16} /></span><span className="loading-text">Ana réfléchit...</span>
                  </span>
                ) : (
      ...
      >
        ...
        <div className="chat-input-wrapper">
          ...
          <button
            ...
            title="Upload: images, docs, code, config, web (64 extensions)"
          >
            <IconUpload size={20} />
          </button>
          <input
            ...
            style={{ display: 'none' }}
            accept=".png,.jpg,.jpeg,.webp,.gif,.bmp,.ico,.svg,.tiff,.tif,.avif,.pdf,.txt,.md,.doc,.docx,.rtf,.odt,.js,.cjs,.mjs,.ts,.tsx,.jsx,.py,.java,.cpp,.c,.h,.hpp,.go,.rs,.rb,.php,.swift,.kt,.scala,.lua,.r,.pl,.sh,.bat,.ps1,.sql,.vue,.svelte,.json,.yaml,.yml,.xml,.ini,.cfg,.conf,.toml,.env,.csv,.tsv,.log,.dat,.html,.htm,.css,.scss,.sass,.less"
            multiple
          />
          <textarea

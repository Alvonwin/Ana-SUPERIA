...
function GamesPage() {
    ...
    const playCheckers = async (move) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setGameState(data);
          if (data.gameOver) {
            addMessage(data.winner === 'player' ? "Tu as gagné!" : "J'ai gagné!");
          }
        } else {
      ...
      <>
        ...
        {gameState.status && gameState.status !== 'playing' && (
          <div className={`game-over ${gameState.status === 'player_wins' ? 'win' : 'lose'}`}>
            {gameState.status === 'player_wins' ? '🎉 Tu gagnes!' : '😢 Ana gagne!'}
          </div>
        )}
      ...
      try {
        ...
        if (data.success) {
          if (data.action === 'first_flip') {
            ...
            setGameState(prev => ({ ...prev, ...data }));
            addMessage("Bien joué! Une paire!");
  ...
  const GuessGame = () => {
    ...
    if (!gameState) {
      return (
        <div className="no-game">
          <p>Ana choisit un nombre entre 1 et 100. À toi de le deviner!</p>
          <button className="btn-new-game" onClick={startGame} disabled={isLoading}>Commencer</button>
        </div>
      );
    }
      ...
      <div className="guess-game">
        ...
        {gameState.status === 'playing' ? (
          ...
          <div className={`game-over ${gameState.status === 'won' ? 'win' : 'lose'}`}>
            {gameState.status === 'won' ? `🎉 Trouvé en ${gameState.attempts} essais!` : `😅 C'était ${gameState.secret}`}
          </div>
        )}
        <button className="btn-new-game" onClick={startGame}>Nouvelle partie</button>
      </div>
  ...
  const ChessGame = () => {
    ...
    if (!gameState) {
      return (
        <div className="no-game">
          <p>Jouer aux Échecs contre Ana!</p>
          <div className="game-controls">
            <select value={difficulty} onChange={e => setDifficulty(e.target.value)}>
              <option value="easy">Facile</option>
              <option value="normal">Normal</option>
  ...
  const BattleshipGame = () => {
    ...
    if (!gameState) {
      return (
        <div className="no-game">
          <p>🚢 Bataille Navale: Place tes 5 bateaux et coule la flotte d'Ana!</p>
          <button className="btn-new-game" onClick={startGame} disabled={isLoading}>Commencer</button>
        </div>
      );
    }

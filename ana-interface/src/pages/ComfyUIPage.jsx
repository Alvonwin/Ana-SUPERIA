...
import { BACKEND_URL, COMFYUI_URL } from '../config';
...
function ComfyUIPage() {
  ...
  // Resize image if too large (> maxSize on any side)
  ...
  // Build AnimateDiff workflow (requires SD1.5 model + AnimateDiff-Evolved)
            ...
            if (nodeOutput.images) {
              return {
                images: nodeOutput.images.map(img => ({
                  url: `${COMFYUI_URL}/view?filename=${encodeURIComponent(img.filename)}&subfolder=${encodeURIComponent(img.subfolder || '')}&type=${img.type || 'output'}`,
    ...
    <div className="comfyui-page">
      ...
      <header className="comfyui-header">
        <div className="header-title">
          ...
          <p className="subtitle">Images et Videos avec Stable Diffusion, AnimateDiff</p>
        </div>
      ...
      <div className="comfyui-content">
        ...
        <div className="results-panel">
          ...
          <div className="main-result">
            {selectedImage ? (
              <>
                {selectedImage.isVideo || selectedImage.filename?.match(/\.(gif|mp4|webm)$/i) ? (

...
class MemoryManager {
  ...
  loadContext() {
    ...
    const maxBytes = MAX_ANA_CONTEXT_KB * 1024;

    try {
      let anaContext = '';

      // Conversations Ana (les plus recentes)
      if (fs.existsSync(this.anaContextPath)) {
        anaContext = fs.readFileSync(this.anaContextPath, 'utf8');
        const anaOriginal = anaContext.length;

        if (anaContext.length > maxBytes) {
          anaContext = anaContext.slice(-maxBytes);
      ...
      this.currentContext = anaContext ? '=== CONVERSATIONS AVEC ANA ===\n' + anaContext : '';
      console.log(`[MEMORY] ANA CONTEXT TOTAL: ${(this.currentContext.length / 1024).toFixed(2)} KB`);
    } catch (error) {

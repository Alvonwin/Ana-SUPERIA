...
async function playCheckers(args, sessionId = 'default') {
  const { action, move, difficulty, mode } = args;
  ...
  switch (action) {
    ...
    case 'start': {
      ...
      const result = checkersEngine.newGame(sessionId, difficulty || 'normal', mode || 'vsAna');
      return {
        success: true,
        reaction: mode === 'vsHuman' ? "Partie 2 joueurs! Que le meilleur gagne! 🎮" : randomReaction('gameStart'),
        ...result
      };
    }

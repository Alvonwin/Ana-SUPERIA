...
$urlsJson = "E:\ANA\server\tunnel_urls.json"
...
$frontendProc = Start-Process -FilePath $cloudflared -ArgumentList "tunnel", "--url", "http://localhost:5173" `
    -RedirectStandardError $frontendLog -PassThru -WindowStyle Hidden
...
"@ | Set-Content $urlsFile
    ...
    # Save URLs as JSON for backend API
    $jsonContent = @{
        frontend = $frontendUrl
        backend = $backendUrl
        generated = (Get-Date -Format "yyyy-MM-dd HH:mm:ss")
    } | ConvertTo-Json
    Set-Content $urlsJson -Value $jsonContent
    Write-Host "   JSON saved for Settings page" -ForegroundColor Green
} else {

...
if (content.includes('get_zodiac_sign')) {
  console.log('SKIP: get_zodiac_sign déjà présent');
  process.exit(0);
}

// 1. Ajouter à la liste des outils valides
...
console.log('OK: Ajouté à la liste des outils valides');

// 2. Ajouter la définition de l'outil après statistics
const statisticsDefEnd = `name: 'statistics',
      description: 'Calculer des statistiques sur une liste de nombres.',
      parameters: {
        type: 'object',
        properties: {
...
const zodiacDef = `name: 'statistics',
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Déterminer le signe astrologique à partir d\\'une date de naissance.',
      parameters: {
        type: 'object',
        properties: {
...
console.log('OK: Définition de l\\'outil ajoutée');

// 3. Trouver TOOL_IMPLEMENTATIONS et ajouter l'implémentation après statistics
const zodiacImpl = `
  // Signe astrologique
  async get_zodiac_sign(args) {
    ...
    const signs = [
      ...
      { name: 'Bélier', start: [3, 21], end: [4, 19], element: 'Feu', emoji: '♈' },
      { name: 'Taureau', start: [4, 20], end: [5, 20], element: 'Terre', emoji: '♉' },
      { name: 'Gémeaux', start: [5, 21], end: [6, 20], element: 'Air', emoji: '♊' },
    ...
    for (const sign of signs) {
      ...
      // Cas spécial Capricorne (chevauche l'année)
      if (sign.name === 'Capricorne') {
        if ((month === 12 && day >= 22) || (month === 1 && day <= 19)) {
          return {
            ...
            message: \`Le \${day}/\${month}, le signe astrologique est \${sign.emoji} \${sign.name} (élément: \${sign.element})\`
          };
        }
      } else if (
    ...
    return { success: false, error: 'Impossible de déterminer le signe' };
  },
`;

// Ajouter après l'implémentation de statistics
content = content.replace(
  /async statistics\(args\) \{[\s\S]*?return \{[\s\S]*?success: true[\s\S]*?\};[\s\S]*?\},/,
  (match) => match + zodiacImpl
);
console.log('OK: Implémentation ajoutée');

// Sauvegarder
fs.writeFileSync(filePath, content, 'utf8');
console.log('DONE: Outil get_zodiac_sign ajouté à tool-agent.cjs');

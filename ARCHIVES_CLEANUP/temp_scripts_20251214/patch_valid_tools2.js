...
// Trouver la ligne avec "Liste des outils valides"
let startLine = -1;
let endLine = -1;
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('Liste des outils valides')) {
    startLine = i;
  }
...
if (startLine === -1 || endLine === -1) {
  console.log('ERROR: Could not find validToolNames block');
  process.exit(1);
}

console.log('Found block from line', startLine + 1, 'to', endLine + 1);

const newBlock = `  // Liste des outils valides - TOUS LES 181 OUTILS (FIX 2025-12-13)
...
// Remplacer
lines.splice(startLine, endLine - startLine + 1, ...newBlock.split('\n'));
fs.writeFileSync(filePath, lines.join('\n'), 'utf8');
console.log('SUCCESS: Replaced validToolNames with all 181 tools');

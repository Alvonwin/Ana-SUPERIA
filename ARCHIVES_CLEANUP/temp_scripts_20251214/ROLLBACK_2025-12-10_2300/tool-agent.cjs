...
`;
  ...
  async port_scan(args) {
    ...
    console.log(`🌐 [ToolAgent] port_scan: ${host} ports=${ports.join(',')}`);
    try {
  ...
  async whois(args) {
    ...
    try {
      const { exec } = require('child_process');
      return new Promise((resolve) => {
        exec(`whois ${domain}`, { timeout: 15000 }, (error, stdout, stderr) => {
...
async function runToolAgent(userMessage, options = {}) {
  ...
  // FIX 2025-12-05: System prompt avec liste d'outils EXPLICITE et règles claires
...
User: "quelle est ma mémoire RAM utilisée" ou "combien de RAM"
...
User: "infos système" ou "système d'exploitation"
